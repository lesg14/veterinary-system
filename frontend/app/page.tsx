  ...
  return (
    <main className={styles.shell}>
      <aside className={styles.sidebar}>
        ...
        <div className={styles.clinicSwitch}><span className={styles.clinicDot} /> ClÃ­nica Central <ChevronRight size={15} /></div>
        ...
        <div className={styles.navBottom}><a className={styles.navItem} href="#configuracion"><Settings2 size={18} /> ConfiguraciÃ³n</a></div>
        <div className={styles.userCard}><div className={styles.avatar}>LG</div><div><strong>Luis Eduardo Soto GarcÃ­a</strong><span>Administradora</span></div><ChevronRight size={16} /></div>
      </aside>
      ...
      <section className={styles.content} id="agenda">
        ...
        <div className={styles.pageHeader}><div><p className={styles.eyebrow}>SÃBADO, 19 DE SEPTIEMBRE DE 2026</p><h1>Agenda diaria</h1><p className={styles.subtitle}>Una vista clara de lo que ocurre hoy en la clÃ­nica.</p></div><button className={styles.primaryButton} onClick={() => setShowNewAppointment(true)}><Plus size={18} /> Nueva cita</button></div>

        <div className={styles.toolbar}><div className={styles.dateControls}><button className={styles.iconButton} onClick={() => moveDate(-1)} aria-label="DÃ­a anterior"><ChevronLeft size={18} /></button><label className={styles.dateField}><CalendarDays size={17} /><input type="date" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} /></label><button className={styles.iconButton} onClick={() => moveDate(1)} aria-label="DÃ­a siguiente"><ChevronRight size={18} /></button></div><label className={styles.filter}><Stethoscope size={16} /><select value={professionalFilter} onChange={(event) => setProfessionalFilter(event.target.value)}><option value="all">Todos los profesionales</option>{agenda?.schedules.map((schedule) => <option key={schedule.professional_id} value={schedule.professional_id}>{schedule.professional}</option>)}</select></label><button className={styles.todayButton} onClick={() => setSelectedDate(toDateInput(new Date()))}>Hoy</button></div>

        <div className={styles.dateTitle}><div><h2>{formatDate(selectedDate)}</h2><span>Horario de atenciÃ³n Â· 08:00 â€” 18:00</span></div><div className={styles.liveStatus}><span /> Agenda actualizada</div></div>
        <section className={styles.stats}><div className={styles.statCard}><div className={`${styles.statIcon} ${styles.coral}`}><CalendarDays size={18} /></div><div><span>Citas confirmadas</span><strong>{scheduledCount}</strong></div><small>para este dÃ­a</small></div><div className={styles.statCard}><div className={`${styles.statIcon} ${styles.mint}`}><Clock3 size={18} /></div><div><span>Espacios libres</span><strong>{freeSlotsCount}</strong></div><small>bloques disponibles</small></div><div className={styles.statCard}><div className={`${styles.statIcon} ${styles.lilac}`}><FileText size={18} /></div><div><span>Atenciones realizadas</span><strong>{attendedCount}</strong></div><small>completadas hoy</small></div></section>

        {loading ? <div className={styles.emptyState}><div className={styles.loader} /><p>Cargando agenda...</p></div> : error ? <div className={styles.errorState}><CircleAlert size={22} /><div><strong>No se pudo conectar con la agenda</strong><p>{error} Verifica que Django estÃ© ejecutÃ¡ndose en el puerto 8000.</p></div><button onClick={loadAgenda}>Reintentar</button></div> : <CalendarBoard agenda={agenda} onAppointmentClick={setSelectedAppointment} />}
      </section>
...
function HistoryModal({ onClose }: { onClose: () => void }) {
  ...
  async function searchHistory(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) throw new Error("No se encontrÃ³ una mascota con ese ID.");
  ...
  return <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className={`${styles.modal} ${styles.historyModal}`} role="dialog" aria-modal="true" aria-labelledby="history-title"><div className={styles.modalHeader}><div><span className={styles.eyebrow}>PACIENTES</span><h2 id="history-title">Historial clÃ­nico</h2></div><button className={styles.iconButton} onClick={onClose} aria-label="Cerrar"><X size={18} /></button></div><form onSubmit={searchHistory} className={styles.historySearch}><label>ID de mascota<input required type="number" min="1" value={petId} onChange={(event) => setPetId(event.target.value)} placeholder="Ej. 1" /></label><button className={styles.primaryButton} disabled={loading}>{loading ? "Consultando..." : "Consultar historial"}</button></form>{error && <div className={styles.formError}><CircleAlert size={16} />{error}</div>}{history && <div className={styles.historyResult}><div className={styles.petSummary}><div className={`${styles.professionalAvatar} ${styles.mint}`}><PawPrint size={17} /></div><div><strong>{history.name}</strong><span>{history.species}{history.breed ? ` Â· ${history.breed}` : ""} Â· {history.vital_status === "DECEASED" ? "Fallecida" : "Viva"}</span></div></div><h3>Atenciones registradas ({history.visits.length})</h3>{history.visits.length === 0 ? <p className={styles.mutedText}>AÃºn no hay atenciones registradas.</p> : history.visits.map((visit) => <div className={styles.visitItem} key={visit.id}><strong>{new Intl.DateTimeFormat("es-CO", { dateStyle: "medium" }).format(new Date(visit.attended_at))}</strong><span>{visit.reason}</span>{visit.diagnosis && <small>{visit.diagnosis}</small>}</div>)}</div>}</section></div>;
}

function ProfessionalSchedule({ schedule, color }: { schedule: Schedule; color: string }) {
  return <article className={styles.professionalBlock}><div className={styles.professionalHeader}><div className={`${styles.professionalAvatar} ${styles[color]}`}>{schedule.professional.split(" ").map((part) => part[0]).join("").slice(0, 2)}</div><div><h3>{schedule.professional}</h3><span><Stethoscope size={14} /> Medicina general</span></div><div className={styles.blockSummary}><strong>{schedule.appointments.length} {schedule.appointments.length === 1 ? "cita" : "citas"}</strong><span>{schedule.free_slots.length} espacios libres</span></div></div><div className={styles.timeline}>{schedule.appointments.length === 0 ? <div className={styles.noAppointments}>Sin citas programadas para este profesional.</div> : schedule.appointments.map((appointment) => <div className={styles.appointmentRow} key={appointment.id}><time>{formatTime(appointment.starts_at)}</time><div className={`${styles.appointmentCard} ${styles[color]}`}><div><strong>{appointment.status === "ATTENDED" ? "AtenciÃ³n registrada" : "Consulta veterinaria"}</strong><span>{statusLabels[appointment.status]} Â· Mascota #{appointment.pet}</span></div><span className={styles.appointmentTime}>{formatTime(appointment.starts_at)} â€” {formatTime(appointment.ends_at)}</span></div></div>)}</div>{schedule.free_slots.slice(0, 3).map((slot) => <div className={styles.freeSlot} key={`${slot.starts_at}-${slot.ends_at}`}><Clock3 size={14} /><span>Disponible</span><time>{formatTime(slot.starts_at)} â€” {formatTime(slot.ends_at)}</time><button aria-label="Agendar en este espacio">Agendar <Plus size={13} /></button></div>)}</article>;
}
...
function AppointmentDetailModal({ appointment, onClose, onSaved }: { appointment: Appointment; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className={styles.modalBackdrop} role="presentation"><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="appointment-detail-title"><div className={styles.modalHeader}><div><span className={styles.eyebrow}>DETALLE DE CITA</span><h2 id="appointment-detail-title">{appointment.pet_name}</h2></div><button className={styles.iconButton} onClick={onClose} aria-label="Cerrar"><X size={18} /></button></div><div className={styles.appointmentDetail}><div><span>Estado</span><strong>{statusLabels[appointment.status]}</strong></div><div><span>Mascota</span><strong>{appointment.pet_name}</strong></div><div><span>Profesional</span><strong>{appointment.professional_name}</strong></div><div><span>Tipo de consulta</span><strong>{appointment.consultation_type_name} Â· {appointment.consultation_duration_minutes} min</strong></div><div><span>Horario actual</span><strong>{formatTime(appointment.starts_at)} â€” {formatTime(appointment.ends_at)}</strong></div></div>{appointment.status === "SCHEDULED" ? <form onSubmit={reschedule} className={styles.form}><label>Nuevo dÃ­a y hora<input required type="datetime-local" value={startsAt} onChange={(event) => setStartsAt(event.target.value)} /></label><small className={styles.modalHint}>La duraciÃ³n se conserva desde el tipo de consulta y el backend volverÃ¡ a validar el solapamiento.</small>{error && <div className={styles.formError}><CircleAlert size={16} />{error}</div>}<button className={styles.primaryButton} disabled={saving}>{saving ? "Guardando..." : "Reagendar cita"}</button></form> : <div className={styles.detailNotice}>Esta cita no estÃ¡ programada y no puede reagendarse.</div>}</section></div>;
}
